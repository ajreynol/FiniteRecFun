...
In this paper, we present a method for translating formulas involving recursive function
...
$\forall x : {\typeint}.\;\, \lite\bigl(
x \leq 0,\allowbreak\; \con{p}( x ) \teq 1,\allowbreak\; \con{p}( x ) \teq 2 * \con{p}( x - 1 ) \bigr)$
can be shown well-founded under an strategy that
...
cover such specifications, based on the observation that infinite unfolding
a corecursive definition computes a unique infinite object.
...
\newcommand\badassex{%
 ...
 \forallf{\con{g}} x.\;\, \lite( \con{f}( x ) \teq 5,\allowbreak\;
...
\begin{longv}
In the following, we sketch the correctness of our translation $\conv$.
...
\begin{longv}
\begin{proofsketch}
First, we show that if $\varphi$ is satisfied by $\Sigma$-interpretation $\I$, then $\conv( \varphi )$ is satisfied by an $\extendsig{\Sigma}$-interpretation $\J$.
...
For example, \isa contains 79 original benchmarks $\varphi$ and 213 translated
...
Admissibility must be checked \relax{by the user}, perhaps using a
...
that it is more efficient to avoid unknown values by adding existential
constraints.
...
early example, based on random testing. Bounded exhaustive
testing\begin{longv} \cite{runciman-et-al-2008}\end{longv} and
narrowing\begin{longv} \cite{lindblad-2008-testing}\end{longv} are other
successful strategies. These tool are often much faster than model finders, but
they typically cannot cope with underspecification and nonexecutable functions.
