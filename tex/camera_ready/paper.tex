...
1$.\end{longv} %ct and it is possible to construct consistent examples that do not meet it.
...
$\fargtype{\tau}$ is an uninterpreted \relax{abstract} type and $\farg{\con{f}} \begin{longv}:
...
\cvc and \ziii for finding counterexamples to verification conditions\begin{nottinyv}
...
%ct changed the following because it is not adequate for our purposes
%
...
Unconventionally, we consider \emph{annotated quantified formulas} of the form
...
In the signature $\Sigma$, we distinguish a subset $\sfundefs{\Sigma} \subseteq \sfuns{\Sigma}$ of \emph{defined} uninterpreted function symbols.
...
$\{\con f:\typeint \rightarrow \typeint,\, \con g:\typeint \rightarrow \typeint,\, \ldots \}$.
...
satisfying $\con{f}$'s definition. A more subtle %ct specimen 
example 
is
...
$\varphi$ and $\varphi'$ are \emph{equisatisfiable (in $T$)}.
...
The translation alters $\varphi$ in two ways. First, it restricts the
...
where, if $\vec s = (s_1,\ldots,s_m)$, $\vecfarg{\con{f}}( a ) \teq \vec s$ abbreviates the formula
$\bigwedge_{i=1,\ldots,m} \fargx{\con{f}}{i}(a) \teq s_i$.
These constraints
...
%ct removed this for consistency because we give no rationale for the other cases when we return true
%Since function definitions are a top-level conjunct, %by case analysis on the return values of $\conv$,
%$\chi$ must be $\ltrue$.
%
...
%ct 
% the tuple of uninterpreted functions $\vecfarg{\con{f}}$ has $\vec s$ in its range.
%ct I find the following reformulation clearer
the input tuple $\vec s$ is the preimage of some element in the abstract domain 
$\fargtype{\con f}$.
...
Let $T$ be linear arithmetic with the uninterpreted symbols  $\{ \con{s} : { \typeint \rightarrow \typeint },\, \const{c} : { \typeint } \}$.
...
positive numbers up to some value $\const{c}$ is greater than $100$. It is
...
\end{equation}
%
The universal quantification in the resulting formula~(\ref{eq:ex-after}) is over an uninterpreted
type $\fargtype{\con{s}}$, making~(\ref{eq:ex-after}) amenable to the finite model finding
...
Furthermore, since all occurrences of the quantified variable~$a$ in~(\ref{eq:ex-after}) are 
...
(i.e., satisfying interpretations) are different in general than those of $\varphi$.
...
$\conv(\varphi)$ is equisatisfiable with $\varphi$ for any input $\varphi$.\footnote{
%ct moved in a footnote to avoid breaking the flow
For the previous example, the intuition is that the
interpretation of a term such as $\con{s}( -1 )$, say, is not
...
%function. 
If the original formula~(\ref{eq:ex-before}) is $T$-satisfiable then the translated
...
solvers to find models for conjectures involving recursively defined functions
whose definitions are admissible.
...
Over \emph{unsat} benchmarks, configuration \cvci is the
...
\ziii solves a total of 209 after the translation, whereas it solves only 187 before the
translation.
%
In contrast, \cvcd finds 211 benchmarks unsatisfiable after the translation.
...
arguments, however, are guided by the types of the quantified variables.
...
We have implemented in the development version of \cvc %~\cite{ReyEtAl-1-RR-13}
...
the values of $\con{s}(x)$ for $x = 0,\ldots,14$ are given.
...
We stress that admissibility must be verified \begin{longv}separately \end{longv}\relax{by the user}---e.g., using a
...
by finite subterm-closed substructures.
%The approach finds sound (fragments
...
%quantifiers ranging over datatypes)\end{longv}.
This idiom was extended by Dunets
et al.\ \cite{dunets-et-al-2010}, who presented a translation scheme
for primitive recursion. Their definedness guards play a similar role to our
...
programs. The QuickCheck tool \cite{claessen-hughes-2000} for Haskell is an
...
over \emph{infinite} types, such as integers and algebraic datatypes.
%\end{nottinyv}
...
\bibitem{barrett-et-al-2011}
C.~Barrett, C.~L. Conway, M.~Deters, L.~Hadarean, D.~Jovanovic, T.~King,
  A.~Reynolds, and C.~Tinelli.
\newblock {CVC4}.
\newblock In G.~Gopalakrishnan and S.~Qadeer, editors, {\em CAV 2011}, volume
  6806 of {\em LNCS}, pages 171--177. Springer, 2011.

\bibitem{smtlib25}
C.~Barrett, P.~Fontaine, and C.~Tinelli.
\newblock {The SMT-LIB Standard: Version 2.5}.
\newblock Technical report, The University of Iowa, 2015.
\newblock (Available at \url{http://smt-lib.org}).

\bibitem{baumgartner-et-al-2009}
P.~Baumgartner, A.~Fuchs, H.~de~Nivelle, and C.~Tinelli.
\newblock Computing finite models by reduction to function-free clause logic.
\newblock {\em J. Applied Logic}, 7(1):58--74, 2009.

\bibitem{blanc2013overview}
R.~Blanc, V.~Kuncak, E.~Kneuss, and P.~Suter.
\newblock An overview of the {Leon} verification system---{V}erification by
  translation to recursive functions.
\newblock In {\em Scala {'}13}. ACM, 2013.

\bibitem{blanchette-nipkow-2010}
J.~C. Blanchette and T.~Nipkow.
\newblock Nitpick: A counterexample generator for higher-order logic based on a
  relational model finder.
\newblock In M.~Kaufmann and L.~C. Paulson, editors, {\em ITP 2010}, volume
  6172 of {\em LNCS}, pages 131--146. Springer, 2010.

\bibitem{claessen-hughes-2000}
K.~Claessen and J.~Hughes.
\newblock Quick{C}heck: A lightweight tool for random testing of {H}askell
  programs.
\newblock In {\em ICFP {'}00}, pages 268--279. ACM, 2000.

\bibitem{claessen-sorensson-2003}
K.~Claessen and N.~S{\"o}rensson.
\newblock New techniques that improve {MACE}-style model finding.
\newblock In {\em MODEL}, 2003.

\bibitem{MouraBjoerner07}
L.~de~Moura and N.~Bj{\o}rner.
\newblock Efficient {E}-matching for {SMT} solvers.
\newblock In F.~Pfenning, editor, {\em CADE-21}, volume 4603 of {\em LNCS},
  pages 183--198. Springer, 2007.

\bibitem{de-moura-bjoerner-2008}
L.~de~Moura and N.~Bj{\o}rner.
\newblock {Z3}: {An} efficient {SMT} solver.
\newblock In C.~R. Ramakrishnan and J.~Rehof, editors, {\em TACAS 2008}, volume
  4963 of {\em LNCS}, pages 337--340. Springer, 2008.

\bibitem{dunets-et-al-2010}
A.~Dunets, G.~Schellhorn, and W.~Reif.
\newblock Automated flaw detection in algebraic specifications.
\newblock {\em J.~Autom.\ Reasoning}, 45(4):359--395, 2010.

\bibitem{GeDeM-CAV-09}
Y.~Ge and L.~de~Moura.
\newblock Complete instantiation for quantified formulas in satisfiability
  modulo theories.
\newblock In {\em CAV '09}, volume 5643 of {\em LNCS}, pages 306--320.
  Springer, 2009.

\bibitem{DBLP:conf/itp/JohanssonDB10}
M.~Johansson, L.~Dixon, and A.~Bundy.
\newblock Case-analysis for rippling and inductive proof.
\newblock In {\em {ITP} 2010}, pages 291--306, 2010.

\bibitem{korovin-2013}
K.~Korovin.
\newblock Non-cyclic sorts for first-order satisfiability.
\newblock In P.~Fontaine, C.~Ringeissen, and R.~A. Schmidt, editors, {\em
  FroCoS 2013}, volume 8152 of {\em LNCS}, pages 214--228. Springer, 2013.

\bibitem{krauss-2009-phd}
A.~Krauss.
\newblock {\em Automating Recursive Definitions and Termination Proofs in
  Higher-Order Logic}.
\newblock {Ph.D.}\ thesis, Technische Universit\"at M\"unchen, 2009.

\bibitem{kuncak-jackson-2005}
V.~Kuncak and D.~Jackson.
\newblock Relational analysis of algebraic datatypes.
\newblock In M.~Wermelinger and H.~Gall, editors, {\em ESEC/FSE 2005}. ACM,
  2005.

\bibitem{mccune-1994}
W.~McCune.
\newblock A {D}avis--{P}utnam program and its application to finite first-order
  model search: Quasigroup existence problems.
\newblock Technical report, Argonne National Laboratory, 1994.

\bibitem{reynolds-blanchette-2015-codata}
A.~Reynolds and J.~C. Blanchette.
\newblock A decision procedure for (co)datatypes in {SMT} solvers.
\newblock In A.~Felty and A.~Middeldorp, editors, {\em CADE-25}, LNCS.
  Springer, 2015.

\bibitem{our-report}
A.~Reynolds, J.~C. Blanchette, and C.~Tinelli.
\newblock Model finding for recursive functions in {SMT}.
\newblock Tech.\ report, \url{http://lara.epfl.ch/~reynolds/SMT2015-recfun/},
  2015.

\bibitem{reynolds-kuncak-2015}
A.~Reynolds and V.~Kuncak.
\newblock Induction for {SMT} solvers.
\newblock In D.~D'Souza, A.~Lal, and K.~G. Larsen, editors, {\em {VMCAI} 2015},
  volume 8931 of {\em LNCS}, pages 80--98. Springer, 2014.

\bibitem{ReynoldsTinelliMoura14}
A.~Reynolds, C.~Tinelli, and L.~de~Moura.
\newblock Finding conflicting instances of quantified formulas in {SMT}.
\newblock In {\em FMCAD 2014}, pages 195--202. {IEEE}, 2014.

\bibitem{ReyEtAl-1-RR-13}
A.~Reynolds, C.~Tinelli, A.~Goel, and S.~Krsti\'c.
\newblock Finite model finding in {SMT}.
\newblock In N.~Sharygina and H.~Veith, editors, {\em CAV 2013}, volume 8044 of
  {\em LNCS}, pages 640--655. Springer, 2013.

\bibitem{reynolds-et-al-2013}
A.~Reynolds, C.~Tinelli, A.~Goel, S.~Krsti{\'c}, M.~Deters, and C.~Barrett.
\newblock Quantifier instantiation techniques for finite model finding in
  {SMT}.
\newblock In M.~P. Bonacina, editor, {\em CADE-24}, volume 7898 of {\em LNCS},
  pages 377--391. Springer, 2013.

\bibitem{slaney-1994}
J.~K. Slaney.
\newblock {FINDER}: Finite domain enumerator system description.
\newblock In A.~Bundy, editor, {\em CADE-12}, volume 814 of {\em LNCS}, pages
  798--801. Springer, 1994.

\bibitem{torlak-jackson-2007}
E.~Torlak and D.~Jackson.
\newblock Kodkod: A relational model finder.
\newblock In O.~Grumberg and M.~Huth, editors, {\em TACAS 2007}, volume 4424 of
  {\em {LNCS}}, pages 632--647. Springer, 2007.

\bibitem{weber-2008}
T.~Weber.
\newblock {\em SAT-Based Finite Model Generation for Higher-Order Logic}.
\newblock {Ph.D.}\ thesis, Technische Universit\"at M\"unchen, 2008.

\bibitem{zhang-zhang-1995}
J.~Zhang and H.~Zhang.
\newblock {SEM}: A system for enumerating models.
\newblock In C.~S. Mellish, editor, {\em IJCAI-95}, volume~1, pages 298--303.
