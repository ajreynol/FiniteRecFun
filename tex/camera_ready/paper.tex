...
and each variable $x:\tau$ of $\svars{\Sigma}$ as an element of $\tau^\I$.
...
(that is, for every $I \in \Mo$, every $\Sigma$-interpretation that differs
...
We refer to the symbols of $\Sigma_2$ that are not is $\Sigma_1$ as \emph{uninterpreted}.
For the rest of the paper \emph{we fix a theory $T = (\Sigma, \Mo)$ 
with uninterpreted symbols} constructed as above.

We will consider \emph{annotated quantified formulas} of the form
...
$\con p:\typeint \rightarrow \Bool$ can be shown well-founded under a strategy that
...
For convenience, it
has been implemented in the development version of \cvc %~\cite{ReyEtAl-1-RR-13}
...
the values of $\con{s}$ for $x = 0,\ldots,14$ are given.
...
Admissibility must be checked \begin{longv}separately \end{longv}\relax{by the user}---e.g., using a
...
techniques,
%introduced by Reynolds et al.\ \cite{ReyEtAl-1-RR-13,reynolds-et-al-2013},
%%% Great work, but let's not cite it more than 4 times if it can be avoided. --jb
...
\newblock The {SMT-LIB} standard---{V}ersion 2.5.
\newblock \url{http://smt-lib.org/language.shtml}, to appear.
