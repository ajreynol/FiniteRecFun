...
type $\tau$ and $\Chi$ is a set of formulas encoding additional constraints.
These constraints are necessary if the polarity is $\pnone$; otherwise, $\Chi$
...
$\conv_0( t\typ{\tau},\, p )$ : 
 \begin{itemize}
   \item[] $\mathsf{if}$ $\tau = \Bool$ and $t = \boolop(t_1,\ldots,t_n)$:
    \begin{itemize}
      \item[] $\mathsf{let}$ $( r_i,\, \Chi_i ) = \conv_0( t_i,\, \pol( \boolop,\, i,\, p ) )$ for $i = 1, \ldots, n$;
      \item[] $\mathsf{let}$ $\Chi = \Chi_1 \mathrel\cup \cdots \mathrel\cup \Chi_n$;
      \item[] $\mathsf{let}$ $\chi = \bigwedge \Chi$;
      \item[] $\mathsf{if}$ $p = \ptrue$
      \begin{itemize}
        \item[] $\mathsf{return}$ $( ( r_1 \boolop \ldots \boolop r_n ) \wedge \chi,\, \emptyset )$
      \end{itemize}
      ...
      \item[] $\mathsf{else}$
      \begin{itemize}
        \item[] $\mathsf{return}$ $( r_1 \boolop \ldots \boolop r_n,\, \Chi )$
      \end{itemize}
    \end{itemize}
