...
These techniques are generally limited to universally quantified formulas in some restricted fragment.
...
productive functions to universally quantified formulas for which known SMT solving techniques are applicable.
...
We must show that formula
$\varphi \wedge (\forallf{\con{f}_1} \vec x.\; \varphi_1) \wedge \cdots \wedge (\forallf{\con{f}_n} \vec x.\; \varphi_n)$ is also $T$-satisfiable.

Let $\J$ be a model of $\varphi$.

Let $\I$ be the restriction of $\J$ to the function symbols in $\sfuns{\Sigma} -
