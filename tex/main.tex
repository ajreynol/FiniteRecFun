...
confirms that the approach works well on benchmarks from various sources. The
...
Many SMT solvers can reason about quantified formulas using incomplete
instantiation-based methods \cite{MouraBjoerner07,ReynoldsTinelliMoura14}.
...
example where both approaches fail is $\forall n : {\sortint}.\;\, \lite\bigl(
\con{p} \leq 0,\allowbreak\; \con{p}( n ) \teq 1,\allowbreak\; \con{p}( n ) \teq 2 * \con{p}( n - 1 ) \bigr)$.
...
productive corecursive functions. The criterion is not met by inconsistent
definitions such as $\forall n : {\sortint}.\;\, \con{f}(n) \teq \con{f}(n) +
1$, but also by some consistent definitions.

Demand for recursive functions in SMT (new draft includes
...
formulas containing recursive function definitions (Section~\ref{sec:encoding})
that produces formulas for which existing model finding techniques are
applicable. The idea

replace a definition $\forall x : \tau.\; \const{f}(x) \teq t$ by $\forall a :
...
type. For admissible definitions, this preserves satisfiability and
unsatisfiability, while making it possible to find finite models using known
techniques.

We provide experimental evidence that this translation improves the
...
Since $\I$ is a model of $\varphi_0$,
there exists a set of literals $\psi_0$ over the atoms occurring in $\varphi_0$
such that $\I$ satisfies $\psi_0$ and $\psi_0$ entails $\varphi_0$.
\ajr{todo}
