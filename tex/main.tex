...
For instance, the set $\{ \forallf{f} \vec x. f( x ) \teq 0 \}$ is well-specified.
A set of well-specified function definitions need be terminating, e.g.
$\{ \forallf{f} x^\sortint. f( x ) \teq f( x - 1 ) \}$ or even $\{ \forallf{f} x. f( x ) \teq f( x ) \}$ are well-specified.
On the other hand, the set $\{ \forallf{f} x. f( x ) \teq f( x ) + 1 \}$ is not well-specified, 
...
While this set has a model where $f$ and $g$ are interpreted as the constant $5$, it is not well-specified
...
Since $\varphi$ is well-defined with respect to functions defined by $\mathcal{D}$, we know $\varphi$ is of the form $\mathcal{D} \wedge \varphi_0$.
...
then since $\mathcal{D}$ is well-specified,
...
noting that the set $\{ \forall_{s} x\typ{\sortint}. ite( x \leq 0, s(x) \teq 0, s( x ) \teq x + s( x-1 ) ) \}$ is well-specified.
...
%If the functions provided in the input are well-specified, 
