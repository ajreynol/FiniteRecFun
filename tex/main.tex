 ...
 \begin{itemize}
   \item[] $\mathsf{if}$ $\tau = \Bool$ and $t = \boolop(t_1,\ldots,t_n)$:
    \begin{itemize}
      ...
      \begin{itemize}
        \item[] $\mathsf{return}$ $( \boolop(t'_1, \ldots, t'_n) ) \wedge \chi,\, \emptyset )$
      \end{itemize}
  ...
  \item[] $\mathsf{else}$ $\mathsf{if}$ $t \equiv \forallf{\con{f}} \vec x.\;\, t_1$
    \begin{itemize}
      \item[] $\mathsf{let}$ $( t'_1,\, D_1 ) = \conv_0( t_1,\, p )$;
      \item[] $\mathsf{if}$ $\con{f} \in \sfundefs{\Sigma}$
      \begin{itemize}
        %\item[] $\mathsf{assert}( D_1 = \emptyset )$;
        \item[] $\mathsf{return}$ $( \forall y\typ{\fargsort{\con{f}}}.\;\, ( t'_1 [ \vecfarg{\con{f}}( y ) / \vec x ] ),\, \emptyset )$
      \end{itemize}
      \item[] else
      \begin{itemize}
        \item[] $\mathsf{return}$ $( \forall \vec x.\; t'_1,\, \forall \vec x.\; D_1 )$
      \end{itemize}
    \end{itemize}
   \item[] $\mathsf{else}$
   \begin{itemize}
