...
$\{ \forallf{\con{s}} x\typ{\typeint}.\;\, \varphi_\con{s} \}$ is admissible,
...
It is easy to see that any interpretation $\I$ satisfying formula~(\ref{eq:ex-after}) must satisfy $\{ \con{s}( \con{c} ) > 100 \}$ 
(call this set $L_0$),
and moreover must satisfy the set $\absconstraints( L_0 ) = \{ \exists a : {\fargtype{\con{s}}}. (\farg{\con{s}}( a ) \teq \con{c}) \}$.
...
The existential $\exists b : {\fargtype{\con{s}}}.\;\, (\farg{\con{s}}( b ) \teq \con{c}-1)$ ensures that a similar constraint is enforced recursively
when $\I$ satisfies $\neg \con{c} \leq 0$.
Hence, given a model of formula~(\ref{eq:ex-after}) that interprets $\con{c}$ as $14$,
this model must satisfy the following formula $\psi$:
%
