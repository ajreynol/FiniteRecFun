...
\cite{GeDeM-CAV-09}
A instantiation-based procedure is shown in~ to be complete for formulas in the \emph{essentially uninterpreted} fragment.
...
For example, the formula $\forall x\typ{\sortint}.\;\, \con{f}( x ) \teq g( x ) + 5$ 
is in the essentially uninterpreted fragment since all occurrences of $x$ occur beneath uninterpreted functions $\con{f}$ and $g$.
...
For example, finite model finding techniques can find a model for
the formula $\forall x\typ{\tau} y\typ{\tau}.\;\, \neg x \teq y \vee \con{f}( x ) \teq \con{f}( y )$
where the uninterpreted type $\tau$ is interpreted as a finite set, and $\con{f}$ as injective finite map.
