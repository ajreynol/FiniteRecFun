...
\begin{itemize}
\item[-] We define a satisfiability-preserving translation method for formulas containing recursive function definitions
...
For instance, the model $\M$ for formula~(\ref{eq:ex-after}) described above interprets $c$ as $14$, 
while there exists a model of formula~(\ref{eq:ex-before}) also interprets $c$ as $14$.
In fact, we claim that there exists a model of $\varphi$ that coincides with a model of $\conv( \varphi )$ on its interpretation of all functions in $\sfuns{\Sigma}$ 
...
We stress that this option is model-sound only if the functions provided in the input are well-specified.
