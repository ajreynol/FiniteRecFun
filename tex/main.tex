...
\emph{productive} co-recursive functions. It is, however, not met by inconsistent
...
$\fargtype{\tau}$ is an uninterpreted \emph{abstract} type and $\farg{\con{f}} :
...
and an if-then-else function symbol $\lite : \Bool \times \tau \times \tau \rightarrow \tau$
for each $\tau \in \stypes{\Sigma}$.
...
$x_1, \ldots, x_n$ of respective type
$\tau_1, \ldots, \tau_n$ and $\varphi$ is a quantifier-free formula 
...
%
where $\con{f}_1, \ldots, \con{f}_n \in \sfuns{\Sigma}$ are distinct function symbols,
%ct added
