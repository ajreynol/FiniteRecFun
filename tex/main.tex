...
$\extendsig{\Sigma}$-interpretation $\I$, then $\varphi$ is satisfied by a
...
such that $\I$ satisfies $L_i \cup \absconstraints( L_i )$ for $i = 0, 1, \ldots\vthinspace$.
...
Since $\I$ satisfies $\conv( \forallf{\con{f}} \vec x.\; \varphi_{\con{f}} )$ and $\absconstraints( L_i )$,
...
first note that by construction $\psi$ entails $\psi_\infty$.
...
Admissibility must be checked separately \relax{by the user}---for example, using a
