...
a formula $\varphi$ 
\ct{I suppose $\varphi$ is meant to be ground.}
is \emph{closed under function expansion 
with respect to $\DDD$} if 
\[
...
$\varphi$ closed under function expansion wrt $\DDD$
the formula
...
\teq 0,\; \con{s}( x ) \teq x + \con{s}( x-1 ) \bigr) \}$ is admissible.
\ct{
...
and easy-to-find counterexamples \ct{contermodels?}. In total, we considered 213 mutated forms of
...
studied many times. 
The text below is partly adapted from Blanchette's Ph.D.\
thesis \cite{blanchette-2012-phd}.
\ct{The last sentence is strange. I would remove it since Jasmin is one of the authors.}
