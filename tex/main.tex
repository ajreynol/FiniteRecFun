...
and $\I$ be a $\extendsig{\Sigma}$-interpretation satisfying $\conv( \varphi )$.
Then, $\I$ satisfies a set of literals $L \cup \absconstraints( L )$ such that $L$ is a set of $\Sigma$-literals that entail $\varphi$.
...
For clarity, we demonstrate this in the context of a concrete example.
...
formula~(\ref{eq:ex-after}) that interprets $\con{c}$ as $14$,
due to the structure of formula~(\ref{eq:ex-after}), it can be shown that $\I$ must satisfy the following formula $\psi$:
%
