...
\title{Model Finding for (Co)recursive Functions in SMT}
...
an (infix) equality predicate $\teq$ of type $\tau \times \tau$ for each $\tau \in \ssorts{\Sigma}$,
...
A \emph{$\Sigma$-interpretation $\I$} %is a mathematical structure that maps
...
and $\forallf{f} \vec x. \varphi$ is eqvuivalent to $\forall \vec x. f( \vec x ) \teq t$ for some $t$.
...
%each $f( \vec t ) \in \terms( M \sigma )$ where $f \in \sfundefs{\Sigma}$, 
%there exists a formula of the form $\exists z\typ{\fargsort{f}}. ( \vecfarg{f}( z ) \teq \vec t )$ in $M \sigma$.
%\end{lemma}
...
Let $\J$ be the $\extendsig{\Sigma}$-interpretation that interprets all sorts $\sigma \in \ssorts{\Sigma}$ as $\sigma^\I$,
...
For instance, the input from Example~\ref{ex:translation} can be written as:
...
Note that this option is model-sound only if the functions provided in the input are well-specified.
