...
$\pnone$. It returns a pair $( t'\negvthinspace,\, D )$, where $t'$ is a term of
type $\tau$ and $D$ is a set of formulas encoding additional constraints.
...
\pol( \boolop, i, p ) =
\begin{cases}
\pnone & \text{if $\boolop = {\teq}$ or $(\boolop, i) = (\lite, 0)$} \\
\neg p & \text{if $\boolop = {\neg}$} \\
p & \text{otherwise}
\end{cases}
\end{equation*}
The negation $\neg p$ of polarity $p$ is $\pfalse$ if $p$ is $\ptrue$, $\ptrue$
if $p$ is $\pfalse$, and $\pnone$ if $p$ is $\pnone$.

If $t$ has positive polarity, we construct a conjunction of the result of the
recursive calls with formulas $D_1, \ldots, D_n$. Similarly, if $t$ has
negative polarity, we construct a disjunction of the result of the recursive
calls with the negation of formulas $D_1, \ldots, D_n$. It $t$ has no polarity,
...
reconstructed with the term $r_1$ returned by the recursive call, and a
