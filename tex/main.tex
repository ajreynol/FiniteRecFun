...
in presence of quantified formulas.
...
Consider the set $\{ \forallf{\con{f}} x.\;\, \con{f}( x ) \teq \con{f}( x ), \forallf{\con{g}} x.\;\, \lite\bigl( \neg \con{f}( x ) \teq 5, \con{g}( x ) \teq \con{g}( x ) + 1, \con{g}( x ) \teq \con{f}( x ) \bigr) \}$.
...
but there exists no interpretation satisfying both $\con{f}( 0 ) \teq 4$ and $\forallf{\con{g}} x.\;\, \lite( \neg \con{f}( x ) \teq 5, \con{g}( x ) \teq \con{g}( x ) + 1, \con{g}( x ) \teq \con{f}( x ) )$.
...
First, we fix a signature $\Sigma$ and a $\Sigma$-formula
...
$\extendsig{ \Sigma }$ and an $\extendsig{ \Sigma }$-formula $\psi$ such that
$\psi$ is satisfiable if and only if $\varphi$ is satisfiable---i.e.,
$\varphi$ and $\psi$ are \relax{equisatisfiable}.
...
\\[-.8\baselineskip]
  ...
  \item[] $\mathsf{else}$ $\mathsf{if}$ $t = \forallf{\con{f}} \vec x.\;\, u$ $\mathsf{then}$
    \begin{itemize}
      ...
      \item[] \return \bigtuple{$\forall a : \fargtype{\con{f}}.\; u' [ \vecfarg{\con{f}}( a ) / \vec x ],\, \ltrue$}
\\[-.8\baselineskip]
    \end{itemize}
...
\\[-.8\baselineskip]
 \begin{itemize}
   \item[] $\mathsf{let}$ $( \varphi',\, \_ ) = \conv_0( \varphi,\, \ppos )$ $\mathsf{in}$
...
the subterms as a conjunction. Dually, if $t$ is negative, the
function constructs a disjunction with the negated constraint. It $t$ has no
polarity, the constrained $\chi$ is simply returned to the caller.
...
If $\varphi$ is a formula not containing function definitions,
and $\I$ is a $\extendsig{\Sigma}$-interpretation satisfying $\conv( \varphi )$,
then $\I$ satisfies a set of literals $L \cup X$ such that
$L$ is a set of $\Sigma$-literals that entail $\varphi$,
and $X$ is the set of literals
$\{ \exists a : {\fargtype{\con{f}}}. ( \vecfarg{\con{f}}( a ) \teq \vec t ) \mid \con{f}( \vec t ) \in \terms( L ), \con{f} \in \sfundefs{\Sigma} \}$.
\end{lemma}
...
To construct $\psi$, we define a sequence of sets of $\extendsig{\Sigma}$-literals $M_0 \subseteq M_1 \subseteq \ldots$ 
such that each $M_i$ is 
satisfied by $\I$ and
can be partitioned into two sets $L_i$ and $X_i$,
where $L_i$ are $\Sigma$-literals,
and $X_i$ consists of literals of the form $\exists a : {\fargtype{\con{f}}}. ( \vecfarg{\con{f}}( a ) \teq \vec t )$
for each term $\con{f}( \vec t ) \in \terms( L_i )$ where $\con{f} \in \sfundefs{\Sigma}$.
Let $M_0$ be 
a set of literals over atoms occurring in $\conv{ \varphi_0 }$
such that $\I$ satisfies each literal in $M_0$ and $\bigwedge M_0$ entails $\conv( \varphi_0 )$.
By case analysis on the return values of $\conv_0$ 
it can be shown that $M_0$ can be partitioned into $L_0$ and $X_0$ in the way described above.
For each $i \geq 0$,
let $\psi'$ be $\bigwedge \{ \conv( \varphi_f[ \vec t / \vec x ] ) \mid \con{f}( \vec t ) \in \terms( L_i ), f \in \sfundefs{\Sigma} \}$,
where $\varphi_f$ denotes the function definition of $f$ in $\DDD$.
Note that $\I$ also satisfies $\psi'$ since $\I$ satisfies $X_i$.
Let $M_{i+1}$ be the union of $M_0$ and
a set of literals $M$ over atoms occurring in $\psi'$
such that $\I$ satisfies each literal in $M$ and $\bigwedge M$ entails $\psi'$,
where we again may partition $M_{i+1}$ into $L_{i+1}$ and $X_{i+1}$.
Let $M^\ast$ be the limit of this sequence, 
which we may partition into (possibly infinite) sets $X^\ast$ and $L^\ast$ in the same way as above.
Let $\psi$ be the $\Sigma$-formula $\bigwedge L^\ast$.
We know $\psi$ is closed under function expansion with respect to $\DDD$ due to its construction.
...
a similar constraint is enforced recursively: when $\varphi_{\con{s}}[t/x]$ is used in part to evaluate $s( c )$,
any model of $\varphi_{\con{s}}[t/x]$ must also satisfy $\varphi_{\con{s}}[t-1/x]$ when $t > 0$. Given a model $\I$ of
...
\end{quote}
%
to the above problem, \cvc will still produce the same output
