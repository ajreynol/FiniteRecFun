...
\cite{blanchette-et-al-2015-fouco}.

Here, we focus on a fragment of all reasonable recursive--corecursive
specifications, where each function is either well-founded or productive.
Mutual (co)recursion is not catered for, but it can be encoded in terms of
simple (co)recursion.

\begin{definition}
Let $\DDD = \{ \forallf{\con{f}_1} \vec x.\; \varphi_1, \ldots,
\forallf{\con{f}_n} \vec x.\; \varphi_n \}$ be a set of function definitions.
This set forms a \emph{layered }(\emph{co})\emph{recursive specification} if
$\const{f}_i$'s definition may not syntactically refer to $\const{f}_{j}$ for
all $i < j$ and each $\const{f}_i$ is either well-founded recursive
or productive corecursive.
\end{definition}

\begin{definition}\rm
Let $\DDD$ be a set of function definitions for $\sfundefs{\Sigma}$. The set
$\DDD$ is \emph{} if for every
...
Beyond totality, an admissible set can include definitions of non-well-founded
functions---for example,
$\{ \forallf{\con{f}} x : \typeint.\;\, \con{f}( x ) \teq \con{f}( x - 1 ) \}$
or even $\{ \forallf{\con{g}} x.\allowbreak\;\, \con{g}( x ) \teq \con{g}( x ) \}$.
More generally, we conjecture that one can ignore all tail-recursive calls
(i.e., calls that occur on the right-hand side, possibly in some $\lite$ branch)
when proving well-foundedness or productivity, without affecting admissibility.
...
satisfying $\con{f}$'s definition. A more subtle example is given by %the set 
\[\{ 
...
%}
% It is not a question of "should you instead unfold function definitions?" since that's essential what quantifier instantiation does.  
% The question is "is quantifier instantiation a good way of unfolding function definitions?" 
% or "how can we instrument quantifier instantiation to mimic/improve upon existing strategies for function unfolding? --ajr
