...
over $\Sigma'$ into a model of $\mathcal{L}$ over $\Sigma$. The table
from Figure~\ref{fig:pipeline} presents the current pipeline implemented in Nunchaku.
...
\begin{tabular}{|l|p{0.6\textwidth}|}
  ...
  \hline
  Type inference
    & Infer types and checks definitions \\
  \hline
  Type skolemization
  ...
  \hline
  Polarization
    & specialize some predicates into a version used in positive position
      and one used in negative position \\
  \hline
  Unrolling
    & Add a decreasing argument to non-well-founded predicates \\
  \hline
  Skolemization
    & Introduce Skolem symbols for term variables \\
  \hline
  Elimination of Predicates
    & Changes a multi-clauses (co)inductive predicate definition
      into a single recursive function \\
  \hline
  Recursion Elimination
    & The encoding from Section~\ref{sec:encoding} \\
  \hline
  Elimination of Match
    & Transforms pattern-matching expressions into tests and selectors \\
  \hline
  CVC4 Invocation 
    & Call CVC4 to obtain a model \\
  \hline
\end{tabular}
%}}}
\caption{Pipeline of Transformations in Nunchaku}
\label{fig:pipeline}
\end{figure}
