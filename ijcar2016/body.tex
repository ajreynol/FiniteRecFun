...
These methods work well in the context of proving the
unsatisfiability of an input set of formulas, 
but they are of little help for finding models for them
...
In the past few years, a few techniques have been developed to find models for
...
functions, as in $\forall x%\typ{\typeint} --- the type is not important, right?
.\;\, \con{f}( x )
\teq \con{g}( x ) + 5$.
% since all
...
The definitons must meet a semantic criterion to be admissible 
...
$\fargtype{\tau}$ is an \emph{abstract} (uninterpreted) type and $\farg{\con{f}} :
...
Given a term $u$, a variable tuple $\vec x = (x_1^{\tau_1},\ldots,x_n^{\tau_n})$ and 
a term tuple $\vec t = (t_1^{\tau_1},\ldots,t_n^{\tau_n})$, 
we write $u[\vec t/\vec x]$ to denote the result of simultaneously replacing
all occurrences of $x_i$ with $t_i$ in $u$, for each $i=1,\ldots,n$.
...
$\DDD = \{ \forallf{\con{f}_1} \vec x_1.\; \varphi_1, \ldots, \forallf{\con{f}_n} \vec x_n.\; \varphi_n \}$,
...
(abstracting the cross product $\tau_1 \times \cdots \times \tau_n$)
and
%\item
...
polarities of logical connectives, to allow models where the sets
...
Howevever, they produce a model for~(\ref{eq:ex-after})
...
and $\absconstraints( L_0 )$ (see~(\ref{eq:xl})) consists of the single constraint
...
Admissibility must be proven externally by the user---e.g., manually, using a
...
verification conditions on simple Scala programs. These bench\-marks also
...
leads to a net gain of one~satisfiable benchmark each with respect to \cvcf.
...
in Sledgehammer \cite{blanchette-et-al-2013-smt} and 
that to try to use Nunchaku also as a proof tool.
...
where, however, the SMT solver is invoked only once
