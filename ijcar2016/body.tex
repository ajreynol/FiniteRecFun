...
\con{p}( x ) \teq \lite\bigl( x \leq 0,\allowbreak\;  1,\allowbreak\;
  2 * \con{p}( x - 1 ) \bigr)$,
where the theory $T$ is integer arithmetic extended with the %uninterpreted
symbol $\con p:\typeint \rightarrow \typeint$, can be shown to be well founded
...
%
\begin{quote}
\verb$val m := Suc (Suc Z).$ \\
\verb$val odd := fun x. if x = Suc Z then true else ?__.$ \\
\verb$val even := fun x. if x = Suc (Suc Z) || x = Z then true else ?__.$\kern-200mm
\end{quote}
%
...
\end{quote}
%
Nunchaku transforms the specification of \verb$head$ into
%
\begin{quote}
