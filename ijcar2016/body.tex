...
\label{ssec:nunchaku}
  ...
  * Nunchaku
    * intended as a higher-order model finder with a rich input language,
      including (co)datatypes, (co)recursive functions, and
      (co)inductive predicates
  * first version, 0.1, came out in January 2016, without support for
    higher-order constructs yet
  * standalone tool; have a frontend for Isabelle and are envisioning
    frontends for other proof assistants, notably Coq and the
    TLA\textsuperscript{+} proof system
  * displace Nitpick, based on Kodkod
    * CVC4 has built-in (co)datatypes and efficient arithmetic
    * also, Nitpick is intimately tied to Isabelle

  * advantage over Nitpick:
    * generates a single problem, not parameterized by cardinalities
      * pushes all the work to the SMT solver
        * we are just starting with that -- and see lots of potential for
          optimizations and tuning

  * flavor of input language
    * case expressions

  * architecture: pipeline of transformations, each of which consists of a
    pair of functions $(translate, reconstruct)$:
    * translates transforms a problem over a signature $\Sigma$ in a logic $\mathcal{L}$
      to a problem over a signature $\Sigma'$ in a logic $\mathcal{L}'$
    * reconstruct transforms a model back from $\Sigma'$ to $\Sigma$
    * similar to institutions \cite{goguen-meseguer}

  * table with the current pipeline

  * rec transformation is just one operation in pipeline
    * reconstruct function: transforms a total model of the encoded problem
      into a partial model of the original model, trimming out junk and adding
      "..." to represent missing entries

  * higher-order support will be done as a natural extension of the encoding
