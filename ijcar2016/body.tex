...
with uninterpreted symbols}, constructed as above, and assume it is a Herbrand theory.
...
We construct the signature $\extendsig{\Sigma}$ so that,
...
to translate and a \relax{polarity}~$p$ for $t$, which can be $\ppos$, $\pneg$, or
$\pnone$.
...
In the call $\conv_0(t, p)$, if $t$'s top symbol is a predicate symbol $\boolop$, including the
...
If $t$ is a function definition $\forallf{\con{f}} \vec x.\; u$, % of a function in $\sfundefs{\Sigma}$,
...
For the example above, \cvc outputs a %partial
...
The input syntax was inspired by that of other systems based on higher-order
...
Conceptually, the sequence of transformation is a \relax{bidirectional pipeline}
